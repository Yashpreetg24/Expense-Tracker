...
const updateExpenseValidator = [
  ...
  body('expenseDate').optional().isISO8601().toDate().withMessage('Valid date is required'),
  body('categoryId').optional().isInt().withMessage('Category ID must be an integer'),
  body('paymentMethodId').optional().isInt().withMessage('Payment Method ID must be an integer'),
  (req, res, next) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
